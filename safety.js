...
function renderSafetyFeed() {
  ...
  const blocked = new Set(safety.blockedUserIds), myMatches = state.posts.filter(post => idsFor(post).includes(current.id) && !idsFor(post).some(id => blocked.has(id)));
  ...
  const posts = state.posts.filter(post => { const ids = idsFor(post); if (post.pending || post.approvalStatus === 'declined' || post.contentStatus === 'reviewing' || post.contentStatus === 'rejected' || ids.some(id => blocked.has(id))) return false; if (ids.includes(current.id)) return true; return ids.some(id => connectedSince.has(id) && postTime(post) >= connectedSince.get(id)); }).slice(0, 5);
...
window.openContentMenu = postId => {
  const {current, state} = session(), post = state.posts.find(item => item.id === postId);
  if (!post) return;
  window.safetyTarget = {targetType: 'post', postId};
  // A post may have been submitted either by the current player or by their
  // opponent. Offer every other participant, not just the author, as a block
  // target so the menu works in both directions (and for doubles).
  const targets = [...new Set(idsFor(post).filter(uid => uid && uid !== current?.id))]
    .map(uid => ({uid, name: state.users.find(user => user.id === uid)?.name || 'player'}));
  const blockActions = targets.map(target => `<button class="secondary wide" onclick="blockUser('${target.uid}')">Block ${escapeHtml(target.name)}</button>`).join('');
  document.getElementById('contentMenuActions').innerHTML = `<button class="secondary wide" onclick="openReportModal()">Report post</button>${blockActions}`;
  document.getElementById('contentMenu').classList.remove('hidden');
};
...
window.deleteMyAccount = async () => { if (document.getElementById('deleteConfirmation').value !== 'DELETE') { alert('Type DELETE to confirm permanent account deletion.'); return; } const button=document.querySelector('#deleteAccountModal .danger'); if(button){button.disabled=true;button.textContent='Deleting account…'} try { await call('deleteMyAccount')({}); try{await signOut(auth)}catch(error){console.warn('Firebase session was already removed',error)} window.closeSafetyModal('deleteAccountModal'); window.closeProfile?.(); window.showLogin?.(); alert('Your Rallysh account and associated personal data have been deleted.'); } catch (error) { console.error(error); alert('Could not delete your account. Please sign in again and retry.'); if(button){button.disabled=false;button.textContent='Permanently delete account'} } };
