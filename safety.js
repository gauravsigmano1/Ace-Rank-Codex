...
window.deleteMyAccount = async () => { if (document.getElementById('deleteConfirmation').value !== 'DELETE') { alert('Type DELETE to confirm permanent account deletion.'); return; } try { await call('deleteMyAccount')({}); await signOut(auth); window.closeSafetyModal('deleteAccountModal'); window.closeProfile?.(); window.showLogin?.(); alert('Your Rallysh account and associated personal data have been deleted.'); } catch (error) { console.error(error); alert('Could not delete your account. Please sign in again and retry.'); } };
